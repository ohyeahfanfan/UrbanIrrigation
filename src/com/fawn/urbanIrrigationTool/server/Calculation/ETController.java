...
public class ETController extends Hydrology{
	...
	public void calculateOneRound(float[] input, float[] output, int roundNum){
		...
		if(roundNum==0){
			...
			output[ETCpre] = input[ETCcur];
		}
		if(output[ETCpre]+output[ETa]-input[R]<0){
			output[ETCcur] = 0; //output[ETC] = ETCi and input[ETC] = ETCi-1
			input[I] = 0;
		}else{
			output[ETCcur] = output[ETCpre]+output[ETa]-input[R];
			if(input[I] > 0){
				input[I] = output[ETCcur]>(input[FC]*input[RD])?(input[FC]*input[RD]):output[ETCcur];
				output[ETCcur] = 0;
			}else{
				input[I] = 0;
				output[ETCcur] = output[ETCpre]+output[ETa]-input[R];
	...
	public static void main(String[] args) {
		ETController h = new ETController();
